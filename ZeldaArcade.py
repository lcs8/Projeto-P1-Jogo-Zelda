...
def main():
    ...
    while running == True:
        ...
        if player.Rect.colliderect(itens.arrow_rect):    
            ARROWS_COLECTED += 1 
            itens.arrow_rect = pygame.Rect(random.randint(200,(WIDTH - 200)), random.randint(200,(HEIGHT - 200)),SPRITES_WIDTH/2, SPRITES_HEIGHT/2)
            itens.Arrow()
            music_arrow.play()
        ...
        if player.Rect.colliderect(itens.enemy_rect):
            if itens.enemy_power < 70:
                ENEMY_COLECTED -= 1
                itens.enemy_rect = pygame.Rect(random.randint(200,(WIDTH - 200)), random.randint(200,(HEIGHT - 200)),SPRITES_WIDTH, SPRITES_HEIGHT)
